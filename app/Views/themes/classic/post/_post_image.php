<?php
$imgAlt = (!empty($postItem->image_alt)) ? $postItem->image_alt : (isset($postItem->title) ? $postItem->title : '');
if (!empty($postItem) && !empty($type)):
    ...
    if ($type == 'featured_slider'): ?>
        ...
        <div class="img-container">
            <img src="<?= IMG_BASE64_600x460; ?>" data-lazy="<?= getPostImage($postItem, 'slider'); ?>" alt="<?= esc($imgAlt); ?>" class="img-cover" width="600" height="460"/>
        </div>
    <?php elseif ($type == 'random_slider'): ?>
        <img src="<?= IMG_BASE64_360x215; ?>" alt="bg" class="img-responsive img-bg" width="360" height="215"/>
        <div class="img-container">
            <img src="<?= IMG_BASE64_360x215; ?>" data-lazy="<?= getPostImage($postItem, "mid"); ?>" alt="<?= esc($imgAlt); ?>" class="img-cover" width="360" height="215"/>
        </div>
    <?php else:
        if ($type == 'featured') {
        ...
        <div class="img-container">
            <img src="<?= $imgBg; ?>" data-src="<?= getPostImage($postItem, $image_size); ?>" alt="<?= esc($imgAlt); ?>" class="lazyload img-cover"<?= !empty($imgSz) ? $imgSz : ''; ?>/>
        </div>
    <?php else: ?>
        <img src="<?= $imgBg; ?>" data-src="<?= getPostImage($postItem, $image_size); ?>" alt="<?= esc($imgAlt); ?>" class="lazyload img-responsive img-post"<?= !empty($imgSz) ? $imgSz : ''; ?>/>
    <?php endif;
    endif;
endif; ?>
