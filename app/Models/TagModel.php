...
class TagModel extends BaseModel
{
    ...
    {
        // only tags from posts in the last 6 months, visible & published
        $dateSixMonthsAgo = date('Y-m-d H:i:s', strtotime('-6 months'));
        return $this->builder
            ->join('posts', 'posts.id = tags.post_id')
            ->select('tags.tag, tags.tag_slug, COUNT(tags.tag_slug) AS count')
            ->groupBy('tags.tag, tags.tag_slug')
            ->where('posts.lang_id', cleanNumber($this->activeLang->id))
            ->where('posts.status', 1)
            ->where('posts.visibility', 1)
            ->where('posts.is_scheduled', 0)
            ->where('posts.created_at >=', $dateSixMonthsAgo)
            ->orderBy('count DESC')
            ->get(15)
            ->getResult();
    }
