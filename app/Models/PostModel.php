...
class PostModel extends BaseModel
{
    ...
    public function buildQuery($langId = null, $fetchContent = false, $joinTags = false, $isPreview = false)
    {
        ...
        if ($fetchContent) {
            ...
            $this->builder->select('posts.id, posts.lang_id, posts.title, posts.pre_title, posts.title_slug, posts.summary, posts.category_id, posts.image_big, posts.image_slider, posts.image_mid, posts.image_small, posts.image_mime, posts.image_storage, posts.image_alt, posts.slider_order, posts.featured_order, posts.post_type, posts.image_url, posts.user_id, posts.pageviews, posts.post_url, posts.updated_at, posts.created_at');
        }
    ...
    {
        // only posts that are explicitly marked as recommended
        $this->buildQuery();
        return $this->builder
            ->where('posts.is_recommended', 1)
            ->orderBy('posts.created_at DESC')
            ->get($this->recommendedPostsLimit)
            ->getResult();
    }

    //get most viewed posts (last 1 month)
    public function getMostViewedPosts($limit)
    {
        $this->buildQuery();
        $dateOneMonthAgo = date('Y-m-d H:i:s', strtotime('-1 month'));
        $this->builder->where('posts.created_at >=', $dateOneMonthAgo);
    ...
    public function getRelatedPosts($categoryId, $postId, $categories)
    {
		...
		return $this->builder
			->where('posts.id !=', cleanNumber($postId))
			->whereIn('posts.category_id', $categoryIds)
			->where('posts.created_at >= NOW() - INTERVAL 3 MONTH')
			->orderBy('RAND()')
			->limit(6)
			->get()
			->getResult();
